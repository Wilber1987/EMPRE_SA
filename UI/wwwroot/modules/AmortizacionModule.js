...
class AmoritizationModule {
    ...
    static calculoAmortizacion = (contrato) => {
        if (contrato.Transaction_Contratos.Catalogo_Clientes == undefined 
            || contrato.Transaction_Contratos.Detail_Prendas == undefined) {
    ...
    static crearCuotas(contrato) {
        ...
        for (let index = 0; index < contrato.Transaction_Contratos.plazo; index++) {
            // @ts-ignore
            const abono_capital = (parseFloat(contrato.Transaction_Contratos.cuotafija_dolares) - (capital * contrato.tasas_interes));
